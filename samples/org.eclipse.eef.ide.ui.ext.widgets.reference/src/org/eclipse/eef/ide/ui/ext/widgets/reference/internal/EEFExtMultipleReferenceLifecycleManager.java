...
public class EEFExtMultipleReferenceLifecycleManager extends AbstractEEFExtReferenceLifecycleManager {
	 ...
	 *            The context adapter
	 */
	public EEFExtMultipleReferenceLifecycleManager(EEFExtReferenceDescription description, EReference eReference, IVariableManager variableManager,
			IInterpreter interpreter, EditingContextAdapter contextAdapter) {
		super(description, eReference, variableManager, interpreter, contextAdapter);
	}
	...
	protected void browseButtonCallback() {
		...
		Object adapter = this.composedAdapterFactory.adapt(self, IItemPropertySource.class);
		if (adapter instanceof IItemPropertySource) {
			IItemPropertySource propertySource = (IItemPropertySource) adapter;
			IItemPropertyDescriptor propertyDescriptor = propertySource.getPropertyDescriptor(self, this.eReference);
			if (propertyDescriptor != null) {
				ArrayList<Object> choiceOfValues = new ArrayList<Object>(propertyDescriptor.getChoiceOfValues(self));
				FeatureEditorDialog dialog = new FeatureEditorDialog(this.tableViewer.getTable().getShell(), new AdapterFactoryLabelProvider(
						this.composedAdapterFactory), self, this.eReference, propertyDescriptor.getDisplayName(self), choiceOfValues);
				dialog.open();

				EList<?> result = dialog.getResult();
				if (result != null) {
					self.eSet(this.eReference, result);
				}
			}
		}
	}
	...
	protected void addButtonCallback() {
		EObject self = this.getSelf();
		IWizard wizard = new EEFExtEObjectCreationWizard(self, eReference, this.contextAdapter);
	...
	protected void removeButtonCallback() {
		...
		for (Object object : objects) {
			EcoreUtil.remove(this.getSelf(), this.eReference, object);
		}
	}
	...
	private EList<?> getValues() {
		Object value = this.getSelf().eGet(this.eReference);
	...
	public void refresh() {
		...
		if (self instanceof EObject) {
			this.tableViewer.setInput(self);
		}
	}
