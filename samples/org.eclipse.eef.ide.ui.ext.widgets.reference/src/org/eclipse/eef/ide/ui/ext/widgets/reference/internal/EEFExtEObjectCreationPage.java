...
/**
 * This page is used to create the a new EObject for the given reference.
 *
 * @author sbegaudeau
 */
public class EEFExtEObjectCreationPage extends WizardPage {
	 ...
	 * The currentEObject.
	 */
	private EObject semanticCandidate;
	 ...
	 *            The editing context adapter
	 */
	public EEFExtEObjectCreationPage(EObject semanticCandidate, EReference eReference, EditingContextAdapter editingContextAdapter) {
	...
	public void createControl(Composite parent) {
		...
		if (this.eReference.isContainment()) {
			...
			this.initializeContainmentInput(this.semanticCandidate, this.eReference);
		} else {
		...
		if (adapter instanceof IEditingDomainItemProvider) {
			...
			for (Object newChildDescriptor : newChildDescriptors) {
				if (newChildDescriptor instanceof CommandParameter) {
					...
					if (this.eReference.getEReferenceType().isSuperTypeOf(commandParameter.getEReference().getEReferenceType())) {
	...
	private List<EReference> getValidContainmentReferences(EObject eObject) {
		...
		for (EStructuralFeature eStructuralFeature : eAllStructuralFeatures) {
			if (eStructuralFeature instanceof EReference) {
				...
				if (reference.isContainment() && this.eReference.getEReferenceType().isSuperTypeOf(reference.getEReferenceType())) {
	...
	private void performFinishMultiValuedContainmentReference(EObject value) {
		this.eContainmentAdd(this.semanticCandidate, this.eReference, value);
	}
	...
	private void performFinistMonoValuedContainmentReference(EObject value) {
		this.semanticCandidate.eSet(this.eReference, value);
	}
