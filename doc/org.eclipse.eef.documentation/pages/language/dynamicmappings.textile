...
* domainClassExpression: This expression will use concepts of the meta-model in order to compute the number of element to take into account. For example, to fulfill our requirement of one text widget for each property with the type EString, you could type the following domainClassExpression @aql:self.eClass().eAllStructuralFeatures@.
...
* predicateExpression: The expression that needs to be validated in order to create the widget of the if. To fulfill our requirement, we would use the expression @aql:eStructuralFeature.eType = ecore::EString@. This expression will thus be executed for all the properties of our object given the fact that we are iteration on all its structural feature with our domain class expression and then for each structural feature, accessible in a variable named eStructuralFeature, we will ensure that the type of the feature is EString. 
