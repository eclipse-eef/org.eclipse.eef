...
* valueExpression: The value expression is used to compute the value to select.
* editExpression: This expression will be executed when the user will select a radio button. The selected value will be accessible using the variable @newValue@.
* candidatesExpression: This expression is used to compute all the radio button to create and the value that will be associated with each of them.
* candidateDisplayExpression: This expression will compute the label of each radio button using the value associated to the button. This value will be accessible using the variable named @candidate@.
...
* style: The style of the radion widget.
