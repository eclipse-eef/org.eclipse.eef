...
public class LegacyPropertySectionRegistry implements IItemRegistry {
	...
	public List<IEEFSectionDescriptor> getPropertySections(String contributorId, String tabId) {
		...
		if (sectionDescriptorProvider != null) {
			...
			for (ISectionDescriptor sectionDescriptor : sections) {
				...
				LegacyPropertySectionItemDescriptor legacySectionDescriptor = new LegacyPropertySectionItemDescriptor(
						sectionDescriptor.getTargetTab(), sectionDescriptor.getFilter(), sectionDescriptor.getSectionClass(),
						sectionDescriptor.getId(), sectionDescriptor.getEnablesFor(), sectionDescriptor.getAfterSection(),
						sectionDescriptor.getInputTypes(), eefTypeMapper);
	...
	public boolean remove(String id) {
		return this.id2descriptors.removeAll(id) != null;
	}
