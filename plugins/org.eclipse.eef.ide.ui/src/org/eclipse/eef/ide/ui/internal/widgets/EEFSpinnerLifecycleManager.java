...
public class EEFSpinnerLifecycleManager extends AbstractEEFWidgetLifecycleManager {
	...
	/**
	 * The minimum value allowed in the spinner text field.
	 */
	private double min;

	/**
	 * The maximum value allowed in the spinner text field.
	 */
	private double max;
	...
	protected void createMainControl(Composite parent, IEEFFormContainer formContainer) {
		...
		Number valueOfMin = getNumberValueOf(this.description.getMinExpression());
		if (valueOfMin != null) {
			this.spinner.setMinimum(valueOfMin.intValue());
			min = this.spinner.getMinimum() / Math.pow(10, this.spinner.getDigits());
		}
		Number valueOfMax = getNumberValueOf(this.description.getMaxExpression());
		if (valueOfMax != null) {
	...
	/**
	 * Get the result of the expression to evaluate as a Number.
	 *
	 * @param expression
	 *            the expression to evaluate.
	 * @return the result of the expression to evaluate as a Number.
	 */
	private Number getNumberValueOf(String expression) {
		try {
	...
	public void aboutToBeShown() {
		...
		this.controller.onNewSpinnerValue(new Consumer<Object>() {
			...
			public void accept(Object value) {
				if (!spinner.isDisposed()) {
					...
					if (!"".equals(display) && !(spinner.getText() != null && spinner.getText().equals(display))) { //$NON-NLS-1$
						try {
		...
		if (!this.spinner.isDisposed() && this.isDirty && shouldUpdateWhileRendering && updateInProgress.compareAndSet(false, true)) {
			try {
				...
				if (textDoubleValue >= this.min && textDoubleValue <= this.max) {
					...
					EEFIdeUiPlugin.INSTANCE
							.log(new Status(IStatus.WARNING, EEFIdeUiPlugin.PLUGIN_ID, Messages.EEFSpinnerLifecycleManager_valueOutOfRange));
					refresh();
				}
				this.isDirty = false;
				this.setStyle();
