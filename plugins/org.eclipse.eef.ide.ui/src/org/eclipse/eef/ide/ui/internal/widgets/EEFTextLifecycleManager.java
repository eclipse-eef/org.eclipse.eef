				...
				Memento.of(text).ifPresent(m -> {
					...
					if (m.appliesTo(this) && userHasUncommitedInput) {
						if (resettingToPreviousReferenceValue) {
							...
							// Can not use MessageDialog.open(kind, parent, title, message, style, dialogButtonLabels)
							// with Neon...
							MessageDialog dialog = new MessageDialog(this.text.getShell(), "Model Changed During Edition", null, //$NON-NLS-1$
									MessageFormat.format(msg, newDisplayText[0], m.userInput), MessageDialog.QUESTION, 0, choices);
							switch (dialog.open()) {
							case 0:
