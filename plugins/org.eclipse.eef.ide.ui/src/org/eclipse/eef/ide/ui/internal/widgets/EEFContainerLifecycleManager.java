...
public class EEFContainerLifecycleManager implements ILifecycleManager {
	 ...
	 */
	private void createWidgetControl(Composite parent, TabbedPropertySheetPage tabbedPropertySheetPage, EEFWidgetDescription eefWidgetDescription,
	 ...
	 */
	private void createDynamicMappingControl(Composite parent, TabbedPropertySheetPage tabbedPropertySheetPage, EEFDynamicMappingFor dynamicMappingFor) {
		...
		if (domainClassExpression != null && domainClassExpression.trim().length() > 0 && switchExpression != null
				&& switchExpression.trim().length() > 0) {
			...
			if (domainClassExpressionResult.success()) {
				for (EObject eObject : domainClassExpressionResult.asEObjects()) {
					...
					switchExpressionVariables.put("self", this.variableManager.getVariables().get("self")); //$NON-NLS-1$//$NON-NLS-2$
