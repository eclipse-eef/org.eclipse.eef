...
import org.eclipse.eef.ide.ui.internal.extension.IItemDescriptor;
...
public class EEFContainerLifecycleManager implements ILifecycleManager {
	...
	private void createWidgetControl(Composite parent, EEFTabbedPropertySheetPage tabbedPropertySheetPage, EEFWidgetDescription eefWidgetDescription,
		...
		} else if (eefWidgetDescription instanceof EEFCustomDescription) {
			EEFCustomDescription eefCustomDescription = (EEFCustomDescription) eefWidgetDescription;

			// Get the custom description contribute by the extension point
			IItemRegistry<IEEFCustomWidgetDescriptionProvider> eefCustomWidgetDescriptionProviderRegistry = EEFIdeUiPlugin.getPlugin()
					.getEEFCustomDescriptionProviderRegistry();
			for (IItemDescriptor<IEEFCustomWidgetDescriptionProvider> itemDescriptor : eefCustomWidgetDescriptionProviderRegistry
					.getItemDescriptors()) {
				...
				if (eefWidgetDescription.getIdentifier().equals(eefCustomWidgetDescriptionProviderId)) {
					IEEFCustomWidgetDescriptionProvider eefCustomWidgetDescriptionProvider = itemDescriptor.getItem();
