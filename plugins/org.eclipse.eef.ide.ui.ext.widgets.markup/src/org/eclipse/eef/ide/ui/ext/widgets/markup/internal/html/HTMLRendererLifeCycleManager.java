...
public class HTMLRendererLifeCycleManager extends AbstractEEFWidgetLifecycleManager {
	...
	protected void createMainControl(Composite parent, IEEFFormContainer formContainer) {
		...
		String editionToolExpression = Optional.ofNullable(this.controlDescription.getEditionToolExpression()).orElse(""); //$NON-NLS-1$
		boolean hasEditButton = !editionToolExpression.trim().isEmpty();
		...
		if (hasEditButton) {
			Composite buttonsComposite = widgetFactory.createFlatFormComposite(composite);
			...
			editButton = widgetFactory.createButton(buttonsComposite, "", SWT.PUSH); //$NON-NLS-1$
	...
	private String aggregateCss(String htmlContent, String cssExpression) {
		Object cssResult = EvalFactory.of(this.interpreter, this.variableManager).evaluate(cssExpression);
		if (cssResult instanceof String) {
			// If CSS result a valid result then the toHTML expression is expected to return on the
			// content of the body
			String cssContent = (String) cssResult;
