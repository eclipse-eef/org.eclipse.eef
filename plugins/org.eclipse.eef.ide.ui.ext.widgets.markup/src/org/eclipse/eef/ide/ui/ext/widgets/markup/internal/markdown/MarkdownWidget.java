...
public class MarkdownWidget {
	 ...
	 */
	private MarkdownStyleFactory styleFactory;
	...
	public MarkdownWidget(Display display, String markdown, boolean multiLine) {
		...
		this.styleFactory = new MarkdownStyleFactory(display);
	}
				...
				if (styledText != null && !styledText.isDisposed() && needUpdateStyles(styleDescription)) {
					display.syncExec(() -> {
						if (styledText != null && !styledText.isDisposed() && needUpdateStyles(styleDescription)) {
							...
							styledText.setStyleRanges(filteredStyle.getStyles().toArray(new StyleRange[filteredStyle.getStyles().size()]));
