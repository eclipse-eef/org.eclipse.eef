...
public class EEFExtMultipleReferenceLifecycleManager extends AbstractEEFExtReferenceLifecycleManager {
	 ...
	 *            The context adapter
	 */
	public EEFExtMultipleReferenceLifecycleManager(EEFExtReferenceDescription description, EObject semanticCandidate, EReference eReference,
			IVariableManager variableManager, IInterpreter interpreter, EditingContextAdapter contextAdapter) {
		super(description, semanticCandidate, eReference, variableManager, interpreter, contextAdapter);
	}
	...
	protected void browseButtonCallback() {
		Object adapter = this.composedAdapterFactory.adapt(this.semanticCandidate, IItemPropertySource.class);
		if (adapter instanceof IItemPropertySource) {
			IItemPropertySource propertySource = (IItemPropertySource) adapter;
			IItemPropertyDescriptor propertyDescriptor = propertySource.getPropertyDescriptor(this.semanticCandidate, this.eReference);
			if (propertyDescriptor != null) {
				ArrayList<Object> choiceOfValues = new ArrayList<Object>(propertyDescriptor.getChoiceOfValues(this.semanticCandidate));
				FeatureEditorDialog dialog = new FeatureEditorDialog(this.tableViewer.getTable().getShell(), new AdapterFactoryLabelProvider(
						this.composedAdapterFactory), this.semanticCandidate, this.eReference,
						propertyDescriptor.getDisplayName(this.semanticCandidate), choiceOfValues);
				dialog.open();

				EList<?> result = dialog.getResult();
				if (result != null) {
					this.semanticCandidate.eSet(this.eReference, result);
				}
			}
		}
	}
	...
	protected void addButtonCallback() {
		IWizard wizard = new EEFExtEObjectCreationWizard(this.semanticCandidate, this.eReference, this.contextAdapter);
	...
	protected void removeButtonCallback() {
		...
		for (Object object : objects) {
			EcoreUtil.remove(this.semanticCandidate, this.eReference, object);
		}
	}
	...
	private EList<?> getValues() {
		Object value = this.semanticCandidate.eGet(this.eReference);
	...
	public void refresh() {
		...
		this.tableViewer.setInput(this.semanticCandidate);
	}
